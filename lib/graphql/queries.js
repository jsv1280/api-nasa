...
module.exports = {
    ...
    getSubscribers: async (root,{ first = 0, skip = 0 }) => {
        let db
        let subscribers = []
        try {
            db = await connectDb()
            subscribers = await db.collection('suscripciones').find().limit(first).skip(skip).toArray()
        } catch (error) {
            const { title , message } = error
            errorHandler(title,message)
        }
    ...
    getMemoramas: async (root,{ first = 0, skip = 0 }) => {
        let db
        let memos = []
        try {
            db = await connectDb()
            memos = await db.collection('memorama').find().limit(first).skip(skip).toArray()
        } catch (error) {
            const { title , message } = error
            errorHandler(title,message)
        }
